...
class MLPModel(torch.nn.Module):
    def __init__(self, x_dim, z_dim, hidden_dim=128):
        super().__init__()
        layers = [torch.nn.Linear(x_dim, hidden_dim),
                  torch.nn.ReLU(),
        ...
        self.hidden_to_mu = torch.nn.Linear(hidden_dim, z_dim)
        self.hidden_to_std = torch.nn.Linear(hidden_dim, z_dim)
...
class MLP(torch.nn.Module):
    ...
    def fit(self, use_early_stoping=True):
        ...
        early_stopper = EarlyStopper(patience=100, min_delta=1.0)
