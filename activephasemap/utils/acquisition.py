...
class CompositeModelUncertainity(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        z_mu = self.c_to_z.mu_scaler.inverse_transform(z_mu)
        z_std = self.c_to_z.std_scaler.inverse_transform(z_std)        
        nr, nb, d = z_mu.shape
    ...
    def optimize(self, batch_size, num_restarts=8, n_iterations=100):
